...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    team = [successor.getAgentState(i) for i in self.getTeam(successor)]
    buddy = [a for a in team if a.getPosition() != None and a.getPosition() != successor.getAgentState(self.index).getPosition()]
    if len(buddy) > 0:
      features['buddyDist'] = min(self.getMazeDistance(myPos, mate.getPosition()) for mate in team)
    else: 
      features['buddyDist'] = 0
